...
### Placeholder / fit awaiting derivation
- Flag 42: Sub-Neptune envelope stripping boundary uses placeholder
  envelope mass fraction (5% of M). Initial envelope mass requires
  protoplanetary disk accretion variable.
...
Notes record documented choices, accepted limitations, and sourced
constants. They are recorded for transparency and do not require
follow-up work. They do not appear in the open-work queue.

### Inherent model approximations
- Note 05: Power-law alpha exponents — inherent

- Note 08: Compositional degeneracy — inherent

- Note 11: Uniform density P_c — inherent approximation

- Note 12: Bashi gas-giant 17% overestimate — inherent to model

- Note 20: Kroupa α empirical — inherent

- Note 22: JJ2010 single measurement context — inherent

- Note 48: Sudarsky albedo applied to brown_dwarf regime — internal
  luminosity not represented. Category: inherent model limitation.

- Note 51: P_cmb uniform-density assumption. 9.5% underestimate. Inherent.

- Note 59: Constant T_m(0) across rocky mass range. Inherent to model.

- Note 60: ODE self-regulation fails below Ra_c throughout thermal
  history. Inherent to model for low-mass rocky planets.

- Note 64: Sluggish lid uses mobile lid formula approximation. Inherent.

- Note 68: Kite (2009) parameterization invalid above ~3 M_Earth.
  Dorn et al. (2018), Noack et al. (2017): pressure suppression
  nonlinearly truncates melting column and raises lower-mantle
  viscosity. One-dimensional Kite model overestimates melt on massive
  super-Earths. Flag for review if M_planet > 3 M_Earth and
...
Note 88: CO2 liquid-vapor window (216.58–304.18 K, P_s > 5.185 bar):
  Span-Wagner EOS deferred; engine returns liquid_possible.
  Model limitation.

Note 89: CO2 Antoine lower bound 154.26 K; Clausius-Clapeyron fallback
  below this. ~20% overshoot confirmed at 149 K extrapolation.
  Model limitation.

Note 97: β = 54.74° singularity — s2 = 0, T2 = 0, formula undefined.
  Handled via explicit branch: if T0 > T_f → ice_free; else →
  full_glaciation. Category: model limitation.

Note 98: P2 Legendre truncation — 3–7% error on φ_ice as function
  of obliquity. Not patchable without spatial grid.
  Category: model limitation.

Note 99: Ice-albedo feedback absent. Global mean albedo used at
  ice-edge; local albedo step not modelled. Underestimates ice extent
  near runaway glaciation. Category: model limitation.

Note 100: EBM annual-mean assumption breaks down at e > 0.3.
  Apoastron winters may drive volatile condensation or atmospheric
  collapse. Formula underestimates maximum glaciation extent on highly
  eccentric worlds. Category: model limitation.

Note 104: M² scaling breaks down above ~5 M_☉.
  Model applicability limit.
  File: snow_lines.py

Note 120: t_disk = 5×(M_star/M_☉)^−0.5 Myr — Mamajek (2009),
  Ribas et al. (2015). Multi-stellar empirical fit.
  Model applicability limit at extreme stellar masses.
  File: nebular_ingassing.py

Note 122: No saturation above ~200 GPa — extrapolation beyond
  experimental dataset. Model applicability limit.
  File: oxygen_fugacity.py

Note 123: K1–K6 A, B coefficients — NIST-JANAF. Universal molecular
  thermodynamics. Valid 1200–2500 K only.
  Model applicability limit outside this range.
  File: equilibrium_speciation.py

Note 132: Equilibrium dissolution assumed — convective mixing
  timescale (days) << disk lifetime (Myr). Model applicability limit
  if magma ocean solidifies before disk dispersal.
  File: nebular_ingassing.py

Note 133: N post-veneer overshoot — 80/20 mixing delivers ~6 ppm N
  vs 1–2 ppm MORB-source target. Known Ru isotopic tension.
  Model limitation — not patched.
  File: late_veneer.py

Note 159: column-mean temperature approximation (T_ref = T_eq)
  in τ₀ integration. Model limitation.
  File: tau_zero_compositional.py.

Note 163: gray-model Venus-regime applicability limit. T_surface
  biased low by up to ~15% for τ₀ > 100. Model applicability limit.
  File: venus_shortwave_attenuation.py.

Note 167: F_int formula valid above 30 M_⊕; sub-Neptune cores
  deviate from H/He polytrope (Lopez & Fortney 2014).
  Model applicability limit. File: gas_envelope_internal_heat.py.

Note 173: gray photolysis approximation; full wavelength-resolved
  actinic flux integration requires correlated stellar spectrum
  coupling out of current scope. Model limitation; fallback J(O₃)
  from F_XUV when O₃ absent. File: photolysis_rates.py.
...
Note 72: CO2 Antoine coefficients (sublimation) — Giauque & Egan (1937)
  via NIST. Earth-measured, universal molecular physics.

Note 73: SO2 Antoine coefficients — Stull (1947) via NIST. 4.7%
  discontinuity at 263 K handoff documented; not patched.
  SO2 triple point pressure: NIST value 1670 Pa used; Antoine-derived
  value gives 1440 Pa (14% deviation at boundary — known Antoine limit).

Note 74: CH4, NH3 Antoine coefficients — standard NIST.
  Earth-measured molecular constants, universal.

Note 78: dH_vap H2O 40,650 J/mol at 373 K — NIST.
  Earth-measured molecular constant.

### Solar System / multi-body confirmed
- Note 34: ε = 0.15 XUV heating efficiency — Solar System calibrated only.

Note 103: T_cond values (170 K, 88 K, 45 K) — nebular-pressure
  condensation temperatures. Solar System calibrated.
  File: snow_lines.py

Note 105: k = 0.44 — oligarchic feeding zone width (Kokubo & Ida 1998).
  Solar System calibrated. Earth fallback.
  File: bulk_volatile_fraction.py

Note 117: CI chondrite fractions — Wasson & Kallemeyn (1988), Lodders
  (2003). Multi-meteorite confirmed.
  File: late_veneer.py

Note 153: H-O binary diffusion from Zahnle & Kasting (1986). Chapman-
  Enskog with Lennard-Jones parameters. Earth/Solar-System calibrated.
  File: b12_coefficients.py.

Note 158: procedural regime thresholds (0.5/10 bar, 0.5 CH4, 150 K)
  Solar System calibrated. File: regime_router.py.

Note 162: Venus-class shortwave attenuation regime parameters
  (n=1, τ_rc=1.0). Solar-System calibrated.
  File: venus_shortwave_attenuation.py.

Note 164: Titan tholin shortwave transmission factor f_trans ≈ 0.45.
  Titan-calibrated empirical fallback (Cassini-Huygens in situ).
  File: titan_tholin_anti_greenhouse.py.

Note 165: Jupiter F_int = 5.4 W/m² normalization (Hanel et al. 1981).
  Solar-System calibrated. File: gas_envelope_internal_heat.py.
...
- Note 09: Rocky M-R universality — Earth fallback

- Note 13: Dwarf rho_0 — Earth fallback

- Note 23: τ_min anchor — Earth fallback

- Note 32: Rocky and dwarf semimajor axes use sub-Neptune distribution.
  No separate rocky-only demographic fit exists at required precision.

- Note 33: Hot Jupiter 1% Bernoulli override — Kepler/RV surveys of Sun-like
  stars only. Not confirmed across all stellar mass ranges.

- Note 36: Isotropic obliquity distribution — theoretically motivated,
  unconfirmed observationally for exoplanets.

- Note 37: Beta eccentricity parameters — Kipping (2013) RV survey; confirmed
  across multiple surveys but with scatter.

  ⚠️ Note 38B: empirical GCM calibration. Residual vs Earth: 0.023.

- Note 39: K_c (thermospheric thermal conductivity) is gas-dependent,
  lab-measured. O/N2: 0.05 W/m/K; H2/He: 0.30 W/m/K.
  Source: Banks & Kockarts (1973).

- Note 46: α = 7 (pressure depth factor) theoretically universal from
  ln(P_meso/P_XUV) ≈ ln(10³). Empirically confirmed on Earth and Mars
  only.

- Note 47: Gas giant T_exo is a lower bound only. Giant Planet Energy
  Crisis — internal Joule/auroral heating dominates on stable giants.
  No cascade variable represents this contribution.

- Note 50: χ = 2.44 core-mantle density contrast. Earth-calibrated.

- Note 52: C_p = 1200 J/(kg·K). Earth silicate calibration.

- Note 54: E_a = 300,000 J/mol dry peridotite. Earth-calibrated. Assumes
  dry rheology.

- Note 55: Cᵢ BSE-calibrated isotope concentrations. Not universal.

- Note 56: ρ_crust, Z_crust, P_f, P_o stagnant lid melt constants.
  Earth/Venus analogue calibration.

- Note 58: T_solidus = 1,500 K sub-Neptune magma ocean gate. Earth
  silicate calibration.

- Note 62: A_mob = 0.122 mobile lid prefactor. Earth-calibrated.

- Note 65: T_solidus Simon-Glatzel fit. Earth peridotite DAC calibration.

- Note 67: k = 3.5 W/(m·K) thermal conductivity. Kite et al. use
  4.18. Our value produces R_melt ~36% below Kite calibration target.
  Earth silicate fallback. Category: Earth fallback.

- Note 69: A_0 = E_a/R_g = 36,083 K activation temperature. Inherits
  Flag 54 (E_a Earth-calibrated dry peridotite). Category: Earth
  fallback (derived).

- Note 70: M_V,☉ = 4.756 mag — empirically adopted solar visual
  magnitude used to anchor the IAU 2015 calibration target.
  Not derivable from Category A constants.
  Category: Earth/Solar System fallback.

Note 75: dH_sub H2O 51,000 J/mol — NIST. Earth-measured molecular constant.

Note 76: dH_sub CO2 26,100 J/mol — NIST. Earth-measured molecular constant.

Note 77: dH_sub SO2 24,900 J/mol — NIST. Not returned by research;
  sourced directly from NIST thermochemical tables.

Note 79: Ice Ih Clapeyron slope 1.35e8 Pa — Wagner et al. (1994).
  Intrinsic to H2O crystal structure; Earth-measured, universal.

Note 80: k_crust = 2.5 W/m/K — terrestrial silicate rock. Earth fallback.
  Universal applicability not confirmed.

Note 81: rho_crust = 2800 kg/m³ — terrestrial continental crust.
  Earth fallback.

Note 82: phi_0 = 0.4 — Athy's Law initial porosity, Earth shale/
  sedimentary baseline. Earth fallback.

Note 83: K_comp = 31 MPa — Athy's Law compaction modulus, Earth shale/
  sedimentary baseline. Earth fallback.

Note 84: rho_b = 2500 kg/m³ — Athy's Law bulk crustal density.
  Earth fallback.

Note 85: N_BUDYKO = 2.0 — MCY shape parameter. Earth-fitted; varies
  1.5–3.0 with biology. Abiotic bare-rock default used. Earth fallback.

Note 86: f_Darcy = 0.05 — Darcy-Weisbach friction factor. Validated
  rocky channels: Earth, Mars, Titan. Earth fallback with multi-body
  partial confirmation.

Note 87: A_GLEN = 2.4e-15 kPa⁻³ s⁻¹ — Glen's flow parameter, Earth
  H2O ice (Paterson 1994). Different values required for CO2/CH4 ice.
  Earth fallback.

Note 94: A = 210 W/m² — OLR linear intercept. Budyko (1969).
  Earth-calibrated empirical coefficient. Fails for dense CO2 or
  H2/He atmospheres. Universal applicability not confirmed.
  Category: Earth fallback.

Note 95: B = 2.0 W/m²/K — OLR temperature sensitivity. Earth-
  calibrated. Encodes water-vapour feedback; wrong for non-H2O
  atmospheres. Category: Earth fallback.

Note 101: X_dry = 1e-3 — EH3 enstatite chondrite hydration.
  Solar System meteoritic measurement. Earth fallback.
  File: bulk_volatile_fraction.py

Note 102: R_snow,H2O 2.7 AU coefficient — solar system snow line
  calibration. Solar System specific. Earth fallback.
  File: snow_lines.py

Note 106: X_max values from Lodders (2003) protosolar abundances.
  Solar-metallicity calibration.
  File: bulk_volatile_fraction.py

Note 107: EH3 fractions f_i — Solar System meteoritic measurements.
  Earth fallback.
  File: elemental_partitioning.py

Note 108: K_D,H = 29 at Earth P_cmb — diamond anvil cell + SIMS.
  Lab measurement. Earth fallback.
  File: core_mantle_partitioning.py

Note 109: K_D,C = 107 at Earth P_cmb — experimental petrology.
  Earth fallback.
  File: core_mantle_partitioning.py

Note 110: K_D,N = 14 at Earth P_cmb — experimental petrology.
  Earth fallback.
  File: core_mantle_partitioning.py

Note 111: K_D,S = 100 at Earth P_cmb — thermodynamic models.
  Earth fallback.
  File: core_mantle_partitioning.py

Note 112: K_D,H = 1 at Mars P_cmb (14 GPa) — H approaches neutral
  below 20 GPa. Lab data.
  File: core_mantle_partitioning.py

Note 113: K_D,C = 500 at Mars P_cmb (14 GPa) — C inversely pressure
  dependent. Lab data.
  File: core_mantle_partitioning.py

Note 114: K_D,N and K_D,S pressure scaling at Mars P_cmb not resolved.
  Earth anchor values used as fallback. Earth fallback pending
  low-pressure experimental data.
  File: core_mantle_partitioning.py

Note 115: Late veneer log-normal μ=−2.3, σ=0.3 — N-body Monte Carlo
  calibration. Solar System calibration.
  File: late_veneer.py

Note 116: 80/20 NC/CC mixing ratio — Ru isotopic anomaly constraint.
  Solar System specific.
  File: late_veneer.py

Note 118: k_H = 0.419 ppm/bar — Henry's Law for H2 in peridotitic melt.
  Lab measurement. Earth fallback.
  File: nebular_ingassing.py

Note 119: f_env coefficient 1.06e-5 — Ikoma & Genda (2006).
  Solar-analog calibration. Earth fallback.
  File: nebular_ingassing.py

Note 124: T_sol,0 = 1400 K — anhydrous peridotite solidus.
  Lab measurement. Earth fallback.
  File: melt_fraction.py

Note 125: γ = 100 K/GPa — solidus Clapeyron slope.
  Lab measurement. Earth fallback.
  File: melt_fraction.py
...
Note 127: dF/dP = 0.12 GPa^−1 — Katz et al. (2003).
  Anhydrous peridotite. Earth fallback.
  File: melt_fraction.py

Note 128: ε_stagnant Gaussian — analytical fit to Dorn et al. (2018)
  + Noack et al. (2017). Earth fallback.
  File: melt_fraction.py

Note 130: Walker (1981) weathering constants — W_0=3.3e14 mol/yr,
  P_CO2_0=3e-4 bar, T_0=285 K, exponent=0.3, sensitivity=13.7 K.
  Earth-empirical. Not applicable without liquid water. Earth fallback.
  File: atmospheric_mass.py

Note 131: FeO supply treated as non-limiting — 8 wt% FeO assumed.
  FeO content not a cascade variable. Earth fallback.
  File: nebular_ingassing.py

Note 152: H₂-CO via H₂-N₂ isosteric approximation. Ivakin & Suetin
  (1964) <2% validation per Marrero & Mason (1972). Earth fallback.
  File: b12_coefficients.py.

Note 160: HITRAN 2020 CIA gray-band averages, Earth/Solar-System
  laboratory measurements, 200–400 K validity. Earth fallback.
  File: cia_coefficients.py.

Note 168: high-T C_p adjustment from JANAF tables. Earth fallback.
  File: moist_adiabat_beta.py.

Note 170: T_mesopause approximated as T_skin in Bates thermosphere
  profile. Model approximation. File: t_p_profile.py.

Note 171: photolysis wavelength cutoffs from JPL Data Evaluation
  2020. Earth fallback. File: photolysis_rates.py.

Note 172: effective broadband photolysis cross-sections (gray
  approximation), JPL Data Evaluation, 200–300 K validity.
  Earth fallback. File: photolysis_rates.py.

Note 174: chemical reaction rate constants from JPL Data Evaluation
  grids. Earth-laboratory measured, 200–400 K validity. Earth
  fallback. File: radical_steady_state.py.

Note 174b: default stratospheric O₃ mole fraction when absent from
  V08 speciation. Earth fallback. File: radical_steady_state.py.

Note 175: JPL Arrhenius parameters for OH+H₂S, OH+SO₂. Earth-
  laboratory measured. Earth fallback. File: species_lifetimes.py.

Note 177: Lindzen 1981 gravity-wave breaking K_zz parameterization;
  1–2 orders of magnitude uncertainty for extrasolar atmospheres.
  Earth fallback. File: eddy_diffusion_kzz.py.

Note 178: Kombayashi-Ingersoll Earth-laboratory L_v and reference
  saturation pressure; condensable-specific universal applicability
  not derived. Earth fallback. File: kombayashi_ingersoll_limit.py.

### Survey-scope / applicability documented
- Note 31: a_max ALMA scaling — single-survey approximation; varies between
  star-forming regions. Candidate C rejected (see research path above).

- Note 49: Del Genio (2019) extrapolation at S_ox < 0.3. Category:
  survey-scope limitation.

Note 166: Saturn helium-rain supplement +0.4 W/m². Solar-System
  calibrated; applicability boundary not derived for extrasolar
  gas giants. File: gas_envelope_internal_heat.py.
...
Note 129: ε_mobile = 1.0 — degassing efficiency of melt.
  Idealised upper bound. Earth fallback.
  File: melt_fraction.py

Note 161b: Venus CO₂ 15 µm line term suppressed (CIA continuum
  carries the opacity at 92 bar). Physically defensible regime
  routing decision. File: line_absorption_coefficients.py.

Note 179: gas envelope uses 1 bar reference P_s when cascade P_s
  is None; resulting T_surface is temperature at 1 bar reference,
  not physical surface. Procedural choice for sub-Neptunes/gas
  giants. File: gas_envelope_internal_heat.py.
...
The following flags were on the open list but are RESOLVED or
DUPLICATE per audit v2. They are preserved in their original
scaffold entries and removed from the running open queue.
